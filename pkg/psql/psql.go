...
func Connect(ctx context.Context, opts ...OptionFunc) (*pgxpool.Pool, error) {
	...
	connString := (&url.URL{
		...
		Host:   fmt.Sprintf("%s:%d", "postgres", options.port),
	...
	if options.migrations != nil {
		...
		if options.logger != nil {
			...
			options.logger.Info("db migrations version", zap.Int("version", int(version)))
		}
		...
		if options.logger != nil {
			for _, i := range res {
				options.logger.Info(
					"migration applied",
					zap.String("path", i.Source.Path),
					zap.String("file", i.Source.Path),
					zap.String("duration", i.Duration.String()),
				)
			}
		}
	}
